...
export default function Chat() {
  ...
  async function loadMessages() {
    ...
    try {
      const { data, error } = await supabase
        .from("messages")
        .select("id, user, content, timestamp, color, user_id")
  ...
  const sendMessage = async () => {
    if (!text.trim()) return;
    // optimistic UI: create a temporary message, then reconcile with Supabase row
          ...
          return curr.map((m) =>
            m.id === tempId
              ? {
                  ...
                  timestamp: inserted.timestamp || m.timestamp,
                }
              : m,
          );
        });
        // Reload messages to ensure full consistency
        await loadMessages();
      }
    } catch (e) {
  ...
  async function clearMessagesInDb() {
    try {
      setDbError(null);
      // Delete all rows by matching all valid UUIDs (greater than or equal to the zero UUID).
  ...
  async function deleteMessage(messageId: string) {
    try {
      ...
      if (!data || data.length === 0) {
        setDbError("Delete failed: message was not found in database.");
        await loadMessages();
        return;
      }
      ...
      console.warn("Failed to delete message from DB:", err);
      setDbError("Failed to delete message from database.");
    }
  }
      ...
      <View style={{ flex: 1, width: "90%" }}>
        <ScrollView
          ...
          onContentSizeChange={(_, __) => {
            ...
            if (
              messages.length <= 3 ||
              messages[messages.length - 1]?.user === profileName
            ) {
              try {
                scrollRef.current?.scrollToEnd({ animated: true });
              } catch {}
