    ...
    try {
      if (devVer) {
        const { data, error } = await supabase
          .from("directmessages")
          .select(
            "id, username, usermsg, devicename, sysver, deviceid, istablet, created_at",
          )
      ...
      if (idsForQuery.some((x) => typeof x === "string")) {
        console.warn(
          "Some message IDs are too large for safe integer conversion — querying using strings for those IDs.",
        );
      }
      ...
      const { data, error } = await supabase
        .from("directmessages")
        .select(
          "id, username, usermsg, devicename, sysver, deviceid, istablet, created_at, adminmsg, admintime",
        )
        .in("id", idsForQuery);
      ...
      >
        ...
        <View style={styles.topic}>
          ...
          <Text style={styles.text2}>
            1. Respect
            {"\n"}2. Don&apos;t spam
            {"\n"}3. Everyone has the same access to the chat and events maker.
            Don&apos;t abuse it.
          </Text>
        </View>
          ...
          >
            ...
            <TouchableOpacity
              onPress={async () => {
                ...
                try {
                  const { deviceId, deviceName, isTablet, systemVersion } =
                  ...
                  if (error) {
                    ...
                    try {
                      ...
                      console.warn(
                        "Failed to persist message IDs after insert:",
                        e,
                      );
                    }
          ...
          {contactMessages.length > 0 || devVer ? (
            ...
            >
              <Text
                style={[
                  styles.text2,
                  { fontWeight: "bold", alignSelf: "center", marginBottom: 4 },
                ]}
              >
                Direct Messages
              </Text>
                      ...
                      {m.adminmsg ? (
                        <>
                          ...
                          >
                            ...
                            <Text style={{ color: c.text, fontSize: 10 }}>
                              {m.admintime
                                ? new Date(m.admintime).toLocaleString()
                                : m.created_at
                                  ? new Date(m.created_at).toLocaleString()
                                  : ""}
                            </Text>
        ...
        <View style={[styles.topic, { marginBottom: 12, marginTop: 0 }]}>
          <Text style={[styles.text2, { marginTop: 4 }]}>
            This app is a work in progress, please be patient as we improve it.
          </Text>
        </View>
        {showCopied ? (
          <View
